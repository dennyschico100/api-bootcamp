...
const UseFulController = require('../controllers/Useful');
router.post('/saveFirstTime', async (req, res) => {
  try {
    const wordObj = {
      data: req.body.data,
    };
    const resp = await UseFulController.create(wordObj.data);
    console.log(resp);
    res.send(resp);
  } catch (error) {
    console.log(error);
...
router.post('/', async (req, res) => {
  try {
    ...
    console.log(wordObj);
    const resp = await UseFulController.create(wordObj);
    res.send(resp);
  } catch (error) {
    res.send('ocurrio un error' + error);
  }
